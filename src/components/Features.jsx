...
import {
  ...
  Stethoscope, Sliders, NoAds, Install, Sparkle, Traffic,
} from './Icons.jsx'

const features = [
  { icon: Traffic, title: 'Live traffic inspector', text: 'Read a patched app’s own HTTPS live in a built-in inspector - requests, headers and bodies, WebSocket frames, replay and breakpoints - captured on the device.' },
  { icon: Search, title: 'Inspect: links, secrets & files', text: 'Extract hosts and URLs found in the APK, surface API keys and tokens (values redacted), and browse every file inside - search names and contents.' },
  ...
  { icon: Unlock, title: 'Remove certificate pinning', text: 'On by default. Detects and neutralises pinning and reports which domains were unpinned. Turn it off to keep the app’s existing pinning config unchanged.' },
  ...
  { icon: Logs, title: 'Capture logs', text: 'Adds a tiny companion so TrustAPK can read the app’s own logcat live over localhost - a new file, the app’s own dex untouched.' },
  { icon: Sparkle, title: 'AI-assisted review', text: 'Ask AI about the links and secrets you surface, or export a chosen evidence bundle to your AI assistant for review - ChatGPT, Gemini, Claude, Grok, with an optional custom prompt. (It can also draft a replacement icon from a description.)' },
  ...
  { icon: Install, title: 'Installation, cloning & signing', text: 'On-device v2/v3 signing, split & bundle handling, and cloning that rewrites package identifiers for a separate installation so resource lookups keep working.' },
]
    ...
    <Section id="features">
      <div className="text-center max-w-2xl mx-auto">
        ...
        <p className="mt-4 text-muted-c">
          A complete toolkit for inspecting and reverse-engineering apps you own - traffic interception
          and analysis, plus a full on-device APK explorer and decompiler.
        </p>
      </div>
